...
class ResponsivePreviewAdminTest extends ResponsivePreviewTestBase {
  ...
  public function testDeviceConfiguration() {
    ...
    foreach ($default_devices as $id => $label) {
      $this->assertSession()->elementExists('xpath', '//table//tr//td[text()="' . $label . '"]');
    }
    ...
    $this->drupalPostForm('admin/config/user-interface/responsive-preview/iphone6/edit', $edit, t('Save'));
    ...
    $this->assertSession()->checkboxChecked('entities[iphone6][status]');
